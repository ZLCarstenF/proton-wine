...
NTSTATUS wine_vkAllocateMemory(void *args)
{
    ...
    if ((res = device->funcs.p_vkAllocateMemory(device->device, &allocate_info_host, NULL, &object->dev_mem)) == VK_SUCCESS)
    {
        if (object->handle == INVALID_HANDLE_VALUE && export_info && export_info->handleTypes & VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT)
        {
            ...
            {
                FIXME("[wine_vkAllocateMemory] vkGetMemoryFdKHR success: %x\n", fd);
...
NTSTATUS wine_vkCreateSharedHandle(void *args)
{
    ...
    FIXME("[wine_vkCreateSharedHandle] Calling device[%p]->funcs.p_vkGetMemoryFdKHR[%p] memory[%llx]->dev_mem[x]\n", device, device->funcs.p_vkGetMemoryFdKHR, memory, object->dev_mem);
    if (device->funcs.p_vkGetMemoryFdKHR(device->device, &get_fd_info, &fd) == VK_SUCCESS)
    {
        FIXME("=====wine_vkCreateSharedHandle SUCCESS=====\n");
        FIXME("[wine_vkCreateSharedHandle] vkGetMemoryFdKHR success: %x\n", fd);
        *outFD = fd;
    }
    else
    {
    ...
    FIXME("[wine_vkCreateSharedHandle] END\n");
